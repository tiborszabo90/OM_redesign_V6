...
import { PawPrint, ChevronRight, ArrowUpRight, ArrowRight, Search, ShoppingBag, History, X, Loader2, Image as ImageIcon, ChevronLeft } from 'lucide-vue-next'
...
const genPool = ['/picbear/people-latte.jpg', '/picbear/people-kitchen.jpg', '/picbear/lattes-plants.jpg', '/picbear/pouring.jpg', '/picbear/machine-counter.jpg', '/picbear/cold-brew.jpg']

// Simulate a larger selected catalog: every selected product is in the list,
// the first `count` already generated, the rest still waiting (status 'pending').
if (variation.value.selectedTotal == null) {
  variation.value.selectedTotal = 100
}
if (!variation.value.products) {
  variation.value.products = variationProducts(variation.value.count)
}
if (variation.value.products.length < variation.value.selectedTotal) {
  for (let i = variation.value.products.length; i < variation.value.selectedTotal; i++) {
    const base = products[i % products.length]
    variation.value.products.push({
      id: `sel-${i}`,
      name: base.name,
      before: base.img,
      after: genPool[i % genPool.length],
      status: 'pending',
    })
  }
}
const detailProducts = computed(() => variation.value.products)
const selectedTotal = computed(() => variation.value.selectedTotal)

const device = ref('desktop')
// Only generated products can be previewed on the storefront.
const previewProducts = computed(() => detailProducts.value.filter(p => p.status === 'done'))
const selectedProductId = ref(previewProducts.value[0]?.id ?? detailProducts.value[0]?.id ?? null)
const selectedProduct = computed(() =>
  products.find(p => p.id === selectedProductId.value) || products[0],
)

const total = computed(() => selectedTotal.value)
...
const generating = computed(() => !!variation.value.generating)

// Selected products that still have no image (never generated, not in a batch).
const remainingCount = computed(() => detailProducts.value.filter(p => p.status === 'pending').length)

// Paginate the product list — a real store can have hundreds of products.
const PAGE_SIZE = 10
const page = ref(1)
const totalPages = computed(() => Math.max(1, Math.ceil(detailProducts.value.length / PAGE_SIZE)))
const pagedProducts = computed(() => {
  const start = (page.value - 1) * PAGE_SIZE
  return detailProducts.value.slice(start, start + PAGE_SIZE)
})
function goPage(p) { page.value = Math.min(Math.max(1, p), totalPages.value) }

// "Generate more" batch control — mirrors the Generate images limit.
const genScope = ref(25)
const genCustom = ref(50)
const genScopeOptions = computed(() => [
...
const genLimit = computed(() => {
  const cap = remainingCount.value
  if (genScope.value === 'all') return cap
  if (genScope.value === 'custom') {
    const n = Math.floor(Number(genCustom.value) || 0)
    return Math.min(Math.max(n, 1), cap)
  }
  return Math.min(genScope.value, cap)
})

function generateMore() {
  const n = genLimit.value
  if (n <= 0 || generating.value) return
  const pending = variation.value.products.filter(p => p.status === 'pending').slice(0, n)
  pending.forEach((p, i) => { p.status = i === 0 ? 'generating' : 'queued' })
  variation.value.generating = true
  startTimer()
}
...
onMounted(() => { if (variation.value.generating) startTimer() })
...
<template>
  <div class="max-w-[960px] mx-auto px-6 py-5">
    ...
    <div class="pb-card p-4 mb-4">
      <div class="grid grid-cols-5 gap-3">
        ...
        <div class="rounded-lg border border-[#e3e3e3] p-3">
          ...
          <p class="text-[14px] font-medium text-[#1a1a1a]">{{ generated }} / {{ selectedTotal }}</p>
        </div>
        <div class="rounded-lg border border-[#e3e3e3] p-3">
    ...
    <div class="pb-card p-4 mb-4">
      <div class="flex items-center gap-3 mb-3">
        ...
        <select v-model="selectedProductId" class="text-[13px] border border-[#d4d4d4] rounded-lg px-2.5 py-1.5 bg-white text-[#1a1a1a] cursor-pointer">
          <option v-for="p in previewProducts" :key="p.id" :value="p.id">{{ p.name }}</option>
        </select>
      </div>
    ...
    <div v-if="remainingCount > 0" class="pb-card p-4 mb-4">
      <p class="text-[13px] font-semibold text-[#1a1a1a] mb-1">Generate more products</p>
      <p class="text-[12px] text-[#6d7175] mb-3">
        {{ remainingCount }} selected product{{ remainingCount !== 1 ? 's' : '' }} in this variation don't have an image yet. Generate the next batch.
      </p>
      <div class="flex items-center gap-2 flex-wrap">
        <button
          v-for="opt in genScopeOptions" :key="opt.value"
          class="rounded-lg border px-3 py-1.5 text-[13px] cursor-pointer transition-colors disabled:opacity-45 disabled:cursor-default"
          :class="genScope === opt.value ? 'border-[#008060] bg-[#f1f8f5] text-[#008060] font-semibold' : 'border-[#d4d4d4] text-[#1a1a1a] hover:border-[#a0a0a0]'"
          :disabled="generating"
          @click="genScope = opt.value"
        >{{ opt.label }}</button>
      </div>

      <div v-if="genScope === 'custom'" class="flex items-center gap-2 mt-3">
        <input
          type="number" v-model.number="genCustom" min="1" :max="remainingCount" :disabled="generating"
          class="w-28 px-3 py-2 rounded-md border border-[#c9cccf] text-[14px] focus:outline-none focus:border-[#5548e0] disabled:opacity-45"
        />
      ...
      <div class="flex items-center justify-between gap-3 flex-wrap mt-3">
        <p class="text-[12px] text-[#1a1a1a]">
          Generating the next <span class="font-semibold">{{ genLimit }}</span> of {{ remainingCount }} remaining product{{ remainingCount !== 1 ? 's' : '' }}.
        </p>
        <button
          class="pb-btn-primary disabled:opacity-45 disabled:cursor-default"
          :disabled="generating || genLimit < 1"
          @click="generateMore"
        >{{ generating ? 'Generating…' : `Generate ${genLimit} more` }}</button>
      </div>
    </div>
      ...
      <div class="flex flex-col">
        ...
        >
          <div class="flex items-center gap-2 shrink-0">
            ...
            <div class="w-12 h-12 rounded-md overflow-hidden border border-[#e0e0e0] bg-[#f6f6f6] flex items-center justify-center">
              <img v-if="p.status === 'done' || p.status === 'generating'" :src="p.after" alt="" class="w-full h-full object-cover transition-opacity" :class="p.status === 'done' ? 'opacity-100' : 'opacity-40'" />
              <ImageIcon v-else :size="16" class="text-[#c9c9c9]" />
            </div>
          </div>
          <div class="flex-1 min-w-0">
            ...
            <p v-else-if="p.status === 'queued'" class="text-[12px] text-[#6d7175] mt-1">Queued</p>
            <p v-else class="text-[12px] text-[#8a8f96] mt-1">Not generated yet</p>
          </div>
          <div class="flex items-center gap-4 shrink-0">
            ...
            <span
              ...
              :class="variation.status === 'active' ? 'bg-[#d4edda] text-[#155724]' : 'bg-[#fff3cd] text-[#856404]'"
            >{{ variation.status === 'active' ? 'Active' : 'Inactive' }}</span>
            <span v-else class="text-[12px] font-semibold rounded-full px-2.5 py-0.5 bg-[#f1f1f1] text-[#8a8f96]">Pending</span>
          </div>
        </div>
      </div>

      <!-- Pagination -->
      <div v-if="totalPages > 1" class="flex items-center justify-between mt-4 pt-3 border-t border-[#ececec]">
        <span class="text-[12px] text-[#6d7175]">
          Showing {{ (page - 1) * PAGE_SIZE + 1 }}–{{ Math.min(page * PAGE_SIZE, detailProducts.length) }} of {{ detailProducts.length }}
        </span>
        <div class="flex items-center gap-1">
          <button
            class="w-8 h-8 rounded-md border border-[#d4d4d4] flex items-center justify-center cursor-pointer hover:bg-[#f6f6f6] disabled:opacity-40 disabled:cursor-default"
            :disabled="page <= 1" @click="goPage(page - 1)"
          ><ChevronLeft :size="16" /></button>
          <span class="text-[13px] text-[#1a1a1a] px-2">Page {{ page }} of {{ totalPages }}</span>
          <button
            class="w-8 h-8 rounded-md border border-[#d4d4d4] flex items-center justify-center cursor-pointer hover:bg-[#f6f6f6] disabled:opacity-40 disabled:cursor-default"
