...
function generateSample() {
  ...
  setTimeout(() => { phase.value = 'sample-ready' }, 1400)
}
...
<template>
  <div class="max-w-[960px] mx-auto px-6 py-5">
    ...
    <template v-if="phase !== 'batch'">
      ...
      <div class="pb-card p-5">
        ...
        <div class="flex items-center gap-5">
          ...
          <ArrowRight :size="16" class="text-[#8a8a8a] shrink-0" />
          <!-- after -->
          <div class="shrink-0 w-40 h-40 rounded-xl overflow-hidden ring-1 ring-[#dedbf7] relative">
            <div v-if="phase === 'idle'" class="absolute inset-0 border-2 border-dashed border-[#d4d4d4] rounded-xl flex items-center justify-center bg-[#fafafa]">
              <span class="text-[11px] text-[#8a8a8a] text-center px-3">Your sample<br />appears here</span>
            </div>
          ...
          <div class="flex-1 min-w-0">
            <template v-if="phase === 'idle'">
              ...
              <p class="text-[12px] text-[#8a8a8a] mt-2">Takes a few seconds. Nothing is added to your store yet.</p>
            </template>
            ...
            <template v-else>
              <p class="text-[13px] font-semibold text-[#0c6b45] flex items-center gap-1.5 mb-1">
                <Check :size="14" /> Sample ready
              </p>
              <p class="text-[12px] text-[#616161] mb-3">Happy with it? We will apply the same look to the other {{ restCount }} products.</p>
    ...
    <!-- BATCH -->
    <template v-else>
      <div class="h-1.5 bg-[#ececec] rounded-full overflow-hidden mb-4">
      ...
      <div class="grid grid-cols-5 gap-3">
        <div v-for="p in selectedProducts" :key="p.id" class="pb-card overflow-hidden">
          <div class="aspect-square relative">
            <div v-if="state.generated[p.id] !== 'done'" class="absolute inset-0 pb-skeleton"></div>
            <div v-else class="absolute inset-0 pb-fade-in">
              <StyledImage :src="p.img" :overlay="chosenStyle.overlay" ai-tag enhance />
            </div>
          </div>
          <p class="px-2 py-1.5 text-[11px] font-medium text-[#1a1a1a] truncate">{{ p.name }}</p>
        </div>
      </div>
