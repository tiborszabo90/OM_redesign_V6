  ...
  <div class="relative">
      ...
      <div class="list-right flex-1 flex items-center gap-3 min-w-0">
        ...
        <div class="list-info flex-1 min-w-0">
          <div class="flex items-center gap-2 mb-1">
            <h3 class="list-name text-base font-semibold text-om-gray-700">{{ name }}</h3>
            <Tag v-if="testBadge" :variant="testBadge.variant" size="sm">
              <template v-if="testBadge.dot" #icon><span class="w-1.5 h-1.5 rounded-full bg-current" /></template>
              {{ testBadge.label }}
            </Tag>
          </div>
    ...
    >
      ...
      <div class="flex items-start justify-between mb-3">
        <div>
          <div class="flex items-center gap-2 mb-1">
            <h3 class="text-base font-semibold text-om-gray-700">{{ name }}</h3>
            <Tag v-if="testBadge" :variant="testBadge.variant" size="sm">
              <template v-if="testBadge.dot" #icon><span class="w-1.5 h-1.5 rounded-full bg-current" /></template>
              {{ testBadge.label }}
            </Tag>
...
import { Check, TrendingUp, MoreVertical, ChevronRight, ChevronsUp, Minus, ChevronsDown } from 'lucide-vue-next'
...
const props = defineProps({
  ...
  // Optional A/B-test state badge: { label, variant, dot? } — testing is a mode of an experience, not a separate entity.
  testBadge: { type: Object, default: null },
