  ...
  <DashboardLayout active-menu-item="campaigns" @logo-click="handleLogoClick" @menu-click="$emit('menu-click', $event)" :right-panel-collapsed="!isChatOpen">
    <template #content>
      <div class="w-full max-w-[1400px] mx-auto -mt-2">
        ...
        <!-- A/B Test Status Banner + Insight side-by-side -->
        <div class="grid grid-cols-1 min-[1100px]:grid-cols-2 gap-5 mb-5 items-stretch">
        <!-- A/B Test Status Banner -->
        <div class="bg-white rounded-lg shadow-[0_1px_2px_1px_rgb(0_0_0/0.03)] px-8 py-4 min-w-0">
          <div class="flex items-center justify-between gap-6 flex-wrap h-full">
            <!-- Left: status + winner -->
        ...
        <div
          ...
          :key="`b-${opp.id}`"
          class="group bg-white rounded-lg shadow-[0_1px_2px_1px_rgb(0_0_0/0.03)] py-5 px-6 min-w-0"
        >
          <div class="flex items-center gap-3">
            ...
            <div class="flex-1 min-w-0 flex flex-col gap-1">
              ...
              <div class="text-[1rem] font-semibold text-om-gray-700 leading-snug truncate">{{ opp.name }}</div>
            </div>
            <div class="shrink-0">
              <Button variant="secondary" size="sm" @click="emit('navigate-to-opportunity', opp.id)">
                Show more
              </Button>
            </div>
          </div>
