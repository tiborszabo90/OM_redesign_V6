        ...
        <div v-if="genTab === 'selected'">
          ...
          <div class="flex items-start justify-between mb-4">
              ...
              <div class="relative">
                ...
                <div v-if="cpShowFilters" class="absolute top-full left-0 mt-2 bg-white border border-om-gray-200 rounded-xl shadow-lg p-4 z-20 flex flex-col gap-4 w-80">
                  <div class="flex flex-col gap-1.5">
                    <span class="text-sm font-semibold text-om-gray-700">Category is</span>
                    <MultiSelect v-model="cpCategoryIs" :options="cpCategoryOptions" placeholder="All categories" />
                  </div>
                  <div class="flex flex-col gap-1.5">
                    <span class="text-sm font-semibold text-om-gray-700">Category is not</span>
                    <MultiSelect v-model="cpCategoryIsNot" :options="cpCategoryOptions" placeholder="All categories" />
                  </div>
                  <div class="flex flex-col gap-3">
                    <div class="flex items-center gap-2.5">
                      <ToggleSwitch v-model="cpOnlyInStock" />
                      <span class="text-sm text-om-gray-600">Only in stock</span>
                    </div>
                    <div class="flex items-center gap-2.5">
                      <ToggleSwitch v-model="cpOnlyWithTraffic" />
                      <span class="text-sm text-om-gray-600">Only products with traffic</span>
                    </div>
                  </div>
                  <div class="h-px bg-om-gray-100"></div>
                  <div class="flex flex-col gap-1.5">
                    <span class="text-sm font-semibold text-om-gray-700">One-time product list import based on another AI tag's generated results</span>
                    <Dropdown v-model="cpImportTag" :options="cpImportTagOptions" placeholder="Select a tag to import from" />
                  </div>
                  <div class="h-px bg-om-gray-100"></div>
                  <div class="flex flex-col gap-2">
                    <span class="text-sm font-semibold text-om-gray-700">Import CSV</span>
                    <div class="border border-dashed border-om-gray-300 rounded-lg py-6 flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-om-gray-400 transition-colors">
                      <Download :size="20" class="text-om-gray-400" />
                      <span class="text-xs text-om-gray-400">Click or drag &amp; drop CSV file here</span>
                    </div>
                    <a href="#" class="flex items-center gap-1.5 text-xs font-medium text-om-orange-500 hover:text-om-orange-600">
            ...
            <div class="flex items-center gap-2">
              ...
              <div class="relative">
                <Button variant="primary" size="md" icon-only @click.stop="showGenSettings = !showGenSettings">
                  <template #icon><Settings :size="16" /></template>
                </Button>
                <div v-if="showGenSettings" class="fixed inset-0 z-40" @click="showGenSettings = false"></div>
                <div v-if="showGenSettings" class="absolute right-0 top-full mt-2 w-[340px] bg-white rounded-xl shadow-[0_4px_16px_rgba(0,0,0,0.12)] border border-om-gray-100 z-50 overflow-hidden">
                  <!-- Header -->
                  <div class="flex items-center justify-between px-4 py-3 border-b border-om-gray-100">
                    <span class="text-sm font-semibold text-om-gray-700">Generation settings</span>
                    <button class="text-om-gray-400 hover:text-om-gray-700" @click="showGenSettings = false"><X :size="16" /></button>
                  </div>
                  <!-- Step selection -->
                  <div class="px-4 py-1.5">
                    <button
                      ...
                      :key="step.value"
                      class="w-full flex items-center justify-between py-2 text-sm cursor-pointer"
                      :class="genStep === step.value ? 'text-om-orange-500 font-semibold' : 'text-om-gray-700'"
                      @click="genStep = step.value"
                    >
                      <span>{{ step.label }}</span>
                      <span class="tabular-nums" :class="genStep === step.value ? 'text-om-orange-500' : 'text-om-gray-400'">{{ step.count }}</span>
                    </button>
                  </div>
                  <!-- Batch generation -->
                  <div class="px-4 py-3 border-t border-om-gray-100 flex items-center justify-between gap-3" :class="batchAvailable ? '' : 'opacity-50'">
                    <div class="min-w-0">
                      <p class="text-sm font-semibold text-om-gray-700">Batch generation</p>
                      <p class="text-xs text-om-gray-400 mt-0.5">Cheaper but slower. Available only when generating a single step.</p>
                    </div>
                    <ToggleSwitch v-model="batchOn" :disabled="!batchAvailable" class="shrink-0" />
                  </div>
                  <!-- Generate a separate image per (variant grouping) -->
                  <div class="px-4 py-3 border-t border-om-gray-100">
                    <p class="text-sm font-semibold text-om-gray-700 mb-1">Generate a separate image per</p>
                    <p class="text-xs text-om-gray-400 mb-2.5">The highest-ranked attribute each product has. Variants that differ only by other attributes (e.g. Size) share one image.</p>
                    <div class="flex flex-col gap-1.5">
                      <div
                        v-for="(attr, i) in cpGenerateBy"
                        :key="attr"
                        draggable="true"
                        @dragstart="onAttrDragStart(i)"
                        @dragover.prevent
                        @drop="onAttrDrop(i)"
                        class="flex items-center gap-2 bg-om-gray-50 border border-om-gray-200 rounded-lg px-2 py-1.5 cursor-grab active:cursor-grabbing"
                      >
                        <GripVertical :size="14" class="text-om-gray-400 shrink-0" />
                        <span class="text-xs font-semibold text-om-gray-400 tabular-nums w-4 shrink-0">{{ i + 1 }}</span>
                        <span class="text-sm font-medium text-om-gray-700 flex-1">{{ attr }}</span>
                        <button class="text-om-gray-400 hover:text-om-gray-700 shrink-0" @click="removeGenerateAttr(attr)"><X :size="14" /></button>
                      </div>
                      <div v-if="cpAddableGenerateAttrs.length">
                        <Dropdown :model-value="''" :options="cpAddableGenerateAttrs" placeholder="+ Add attribute" size="sm" @update:model-value="addGenerateAttr" />
                      </div>
                    </div>
                    <p class="text-xs text-om-gray-500 mt-2.5">≈ <span class="font-semibold text-om-gray-700">{{ genTotalImages.toLocaleString('en-US') }}</span> images · +{{ genExtraImages.toLocaleString('en-US') }} vs. one per product</p>
                  </div>
                  <!-- Footer help -->
                  <div class="px-4 py-3 border-t border-om-gray-100 bg-om-gray-50">
                    <p class="text-xs text-om-gray-400">Choose which step to generate. A step only runs for products whose previous step is ready.</p>
                  </div>
                </div>
              </div>
          ...
          <!-- Product cards -->
          <div class="flex flex-col gap-3">
            <div v-for="product in genProductsPaged" :key="product.id">
            <div
            ...
            >
              ...
              <div class="flex items-center gap-3 w-1/2 pr-4">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <button
                    v-if="productImageInfo(product).count > 1"
                    class="mt-1.5 inline-flex items-center gap-1 cursor-pointer"
                    @click.stop="toggleVariants(product.id)"
                  >
                    <Tag variant="orange">×{{ productImageInfo(product).count }} · {{ productImageInfo(product).attr }}</Tag>
                    <ChevronDown :size="14" class="text-om-gray-400 transition-transform" :class="isVariantsExpanded(product.id) ? 'rotate-180' : ''" />
                  </button>
                </div>
              </div>
            ...
            <!-- Variant sub-rows: what we'll generate on -->
            <div v-if="isVariantsExpanded(product.id) && productVariantList(product).length" class="mt-2 ml-[60px] pl-4 border-l-2 border-om-gray-100 flex flex-col gap-2">
              <div
                v-for="(value, vi) in productVariantList(product)"
                :key="vi"
                class="bg-white border border-om-gray-200 rounded-xl p-3 flex items-stretch gap-0 cursor-pointer hover:shadow-[0_2px_4px_2px_rgb(0_0_0/0.05)] transition-shadow"
                @click.stop="openVariant(product, vi)"
              >
                <!-- Left: variant thumbnail + label -->
                <div class="flex items-center gap-3 w-1/2 pr-4">
                  <div class="aspect-square h-24 bg-om-gray-100 rounded-lg shrink-0 overflow-hidden border border-om-gray-200">
                    <img src="/product1.jpg" class="w-full h-full object-cover" />
                  </div>
                  <div class="flex-1 min-w-0">
                    <p class="text-sm font-semibold text-om-gray-700">{{ productImageInfo(product).attr }}: {{ value }}</p>
                    <p class="text-xs text-om-gray-400 mt-0.5">{{ product.name }}</p>
                  </div>
                </div>
                <!-- Divider -->
                <div class="w-px bg-om-gray-100 shrink-0 self-stretch"></div>
                <!-- Right: generated 16:9 preview + output + status -->
                <div class="flex items-center gap-4 w-1/2 pl-4">
      ...
      <div v-else-if="showGenerationProduct && selectedProduct" class="w-full max-w-[1400px] mx-auto -mt-3">
        ...
        <div class="flex items-center justify-between gap-4 mb-4">
          ...
          <div class="flex items-center gap-2 shrink-0">
            ...
            <span v-if="variantContext" class="text-sm text-om-gray-400 tabular-nums ml-2">{{ variantContext.index + 1 }} / {{ variantContext.values.length }}</span>
            <span v-else-if="selectedProductIndex >= 0" class="text-sm text-om-gray-400 tabular-nums ml-2">{{ selectedProductIndex + 1 }} / {{ genProductsGenerated.length }}</span>
...
<span>New Variable</span>
      ...
      <div v-else-if="showChooseProducts" class="w-full max-w-[1400px] mx-auto -mt-3">
        ...
        <!-- Variant generation grouping -->
        <div class="bg-white rounded-2xl shadow-[0_2px_8px_0_rgba(0,0,0,0.04),0_1px_2px_0_rgba(0,0,0,0.02)] p-4 mb-3">
          <div class="flex items-start justify-between gap-4 flex-wrap">
            <div class="flex-1 min-w-0">
              <p class="text-sm font-semibold text-om-gray-700 mb-2">Generate a separate image per</p>
              <div class="flex items-center gap-2 flex-wrap">
                <div
                  ...
                  @dragover.prevent
                  @drop="onAttrDrop(i)"
                  class="flex items-center gap-1.5 bg-om-orange-50 border border-om-orange-200 rounded-lg pl-2 pr-1.5 py-1 cursor-grab active:cursor-grabbing"
                >
                  <GripVertical :size="13" class="text-om-orange-400" />
                  <span class="text-xs font-semibold text-om-gray-400 tabular-nums">{{ i + 1 }}</span>
                  <span class="text-sm font-medium text-om-gray-700">{{ attr }}</span>
                  <button class="text-om-gray-400 hover:text-om-gray-700" @click="removeGenerateAttr(attr)">
                    <X :size="13" />
                  </button>
                </div>
                <div v-if="cpAddableGenerateAttrs.length" class="w-44">
                  <Dropdown :model-value="''" :options="cpAddableGenerateAttrs" placeholder="+ Add attribute" size="sm" @update:model-value="addGenerateAttr" />
                </div>
              </div>
              <p class="text-xs text-om-gray-400 mt-2">We use the highest-ranked attribute each product has. Variants that differ only by other attributes (e.g. Size) share one image.</p>
            </div>
            <div class="text-right shrink-0">
              <p class="text-2xl font-semibold text-om-gray-700 tabular-nums">≈ {{ cpTotalImages.toLocaleString('en-US') }}</p>
              <p class="text-xs text-om-gray-400">images · +{{ cpExtraImages.toLocaleString('en-US') }} vs. one per product</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-[0_2px_8px_0_rgba(0,0,0,0.04),0_1px_2px_0_rgba(0,0,0,0.02)] overflow-hidden">
          ...
          >
            <div class="flex items-center gap-3 pr-4">
              ...
              <Tag v-if="productImageInfo(product).count > 1" variant="orange" class="shrink-0">×{{ productImageInfo(product).count }} · {{ productImageInfo(product).attr }}</Tag>
            </div>
...
import { Search, ArrowLeft, ArrowRight, Sparkles, Image as ImageIcon, Cpu, Maximize2, Coins, Wand2, ExternalLink, Type, ArrowUpDown, SlidersHorizontal, Download, X, Check, ChevronDown, ChevronUp, ChevronLeft, ChevronRight, Eye, EyeOff, History, Send, Pencil, GripVertical, Settings } from 'lucide-vue-next'
...
// Generation settings popover (gear)
const showGenSettings = ref(false)
const genStep = ref('all') // 'all' | 'step1'
const genSteps = computed(() => [
  { value: 'all', label: 'All steps', count: genProducts.value.length },
  { value: 'step1', label: 'Step 1 - Step 1', count: genProducts.value.length },
])
const batchAvailable = computed(() => genStep.value !== 'all')
const batchOn = computed({
  get: () => genMode.value === 'batch',
  set: (v) => { genMode.value = v ? 'batch' : 'instant' },
})
...
const variantContext = ref(null) // { product, attr, values, index } while viewing a variant
...
// Open a single variant on its own product-detail page (review + AI Input / Prompt Editor).
const variantTitle = (product, attr, value) => `${product.name} · ${attr}: ${value}`
const openVariant = (product, index) => {
  const info = productImageInfo(product)
  const values = productVariantList(product)
  openProductModal({ ...product, status: 'generated', name: variantTitle(product, info.attr, values[index]) }, 'review')
  variantContext.value = { product, attr: info.attr, values, index }
}
const goToVariant = (i) => {
  const v = variantContext.value
  if (!v || i < 0 || i >= v.values.length) return
  selectedProduct.value = { ...v.product, status: 'generated', name: variantTitle(v.product, v.attr, v.values[i]) }
  variantContext.value = { ...v, index: i }
}
...
const hasPrevProduct = computed(() => variantContext.value ? variantContext.value.index > 0 : selectedProductIndex.value > 0)
const hasNextProduct = computed(() => variantContext.value
  ? variantContext.value.index < variantContext.value.values.length - 1
  : selectedProductIndex.value >= 0 && selectedProductIndex.value < genProductsGenerated.value.length - 1)
const goToPrevProduct = () => {
  if (!hasPrevProduct.value) return
  if (variantContext.value) { goToVariant(variantContext.value.index - 1); return }
...
const cpImportTag = ref('')
const cpImportTagOptions = ['Summer Sale 2026 images', 'Black Friday hero shots', 'New Arrivals lifestyle', 'Clearance banners']

// ── Variant generation grouping (ordered attribute priority) ──
const cpGenerateAttrOptions = ['Color', 'Size', 'Material', 'Pattern']
const cpGenerateBy = ref(['Color'])
const cpAddableGenerateAttrs = computed(() => cpGenerateAttrOptions.filter(a => !cpGenerateBy.value.includes(a)))
const addGenerateAttr = (attr) => { if (attr && !cpGenerateBy.value.includes(attr)) cpGenerateBy.value.push(attr) }
const removeGenerateAttr = (attr) => { cpGenerateBy.value = cpGenerateBy.value.filter(a => a !== attr) }
const cpDragAttrIndex = ref(null)
const onAttrDragStart = (i) => { cpDragAttrIndex.value = i }
const onAttrDrop = (i) => {
  const from = cpDragAttrIndex.value
  if (from == null || from === i) return
  const list = [...cpGenerateBy.value]
  const [moved] = list.splice(from, 1)
  list.splice(i, 0, moved)
  cpGenerateBy.value = list
  cpDragAttrIndex.value = null
}
...
// Mock per-product variant attributes (distinct value counts), deterministic by id.
const cpVariantProfiles = [
  { Color: 3, Size: 4 },
  { Color: 5 },
  { Size: 6 },
  {},
...
const productVariantAttrs = (product) => cpVariantProfiles[product.id % cpVariantProfiles.length]
// Images this product yields = distinct values of the highest-priority attribute it has.
const productImageInfo = (product) => {
  const attrs = productVariantAttrs(product)
  for (const a of cpGenerateBy.value) {
    if (attrs[a] > 0) return { count: attrs[a], attr: a }
  }
  return { count: 1, attr: null }
}
const cpTotalImages = computed(() => cpSortedProducts.value.reduce((sum, p) => sum + productImageInfo(p).count, 0))
const cpExtraImages = computed(() => cpTotalImages.value - cpSortedProducts.value.length)
const genTotalImages = computed(() => genProducts.value.reduce((sum, p) => sum + productImageInfo(p).count, 0))
const genExtraImages = computed(() => genTotalImages.value - genProducts.value.length)

// Mock variant value labels per attribute (for the expandable sub-rows).
const cpVariantValues = {
  Color: ['Black', 'White', 'Red', 'Blue', 'Green', 'Beige'],
  Size: ['XS', 'S', 'M', 'L', 'XL', 'XXL'],
  Material: ['Cotton', 'Leather', 'Linen', 'Wool'],
  Pattern: ['Solid', 'Striped', 'Floral', 'Checked'],
}
// The individual variants a product will generate on (chosen attribute, sliced to count).
const productVariantList = (product) => {
  const info = productImageInfo(product)
  if (info.count <= 1 || !info.attr) return []
  const values = cpVariantValues[info.attr] || []
  return Array.from({ length: info.count }, (_, i) => values[i] ?? `${info.attr} ${i + 1}`)
}
const expandedVariantIds = ref(new Set())
const isVariantsExpanded = (id) => expandedVariantIds.value.has(id)
const toggleVariants = (id) => {
  const s = new Set(expandedVariantIds.value)
  s.has(id) ? s.delete(id) : s.add(id)
  expandedVariantIds.value = s
}
