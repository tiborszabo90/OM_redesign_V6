...
<span class="text-om-gray-600 font-medium">New Variable</span>
      ...
      <div v-else-if="showGeneration" class="w-full max-w-[1400px] mx-auto -mt-3">
        ...
        <div class="flex items-start justify-between gap-4 mb-4">
          <h1 class="text-2xl font-bold text-om-gray-800">AI lifestyle image 1</h1>
          <div class="flex items-center gap-3 shrink-0 mt-1">
            <button
              class="flex flex-col items-end gap-0.5 cursor-pointer group"
              @click="showCreditModal = true"
            >
              <span class="flex items-center gap-1.5 text-sm text-om-gray-500 group-hover:text-om-gray-700 transition-colors">
                <Coins :size="16" class="text-om-gray-400" />
                <span class="font-semibold text-om-gray-700">{{ availableCredits.toLocaleString('en-US') }}</span> credits
              </span>
              <span class="text-[11px] text-om-gray-400">{{ planRemaining.toLocaleString('en-US') }} plan · +{{ additionalCredits.toLocaleString('en-US') }} additional</span>
            </button>
            <Button variant="secondary" size="sm" @click="openBuyModal">
              <template #icon><Plus :size="14" /></template>
              Add credits
            </Button>
          </div>
...
<span>Generation</span>
      ...
      <Modal v-model="showCreditModal" size="2xl">
      <template #header>
        <div class="flex items-center gap-3">
          <div class="w-9 h-9 rounded-xl bg-om-orange-500 flex items-center justify-center shrink-0">
            <Receipt :size="18" class="text-white" />
          </div>
          <h2 class="text-lg font-semibold text-om-gray-700">Credit Transactions</h2>
        </div>
      </template>

      <!-- Balance summary: plan (renews) + additional (never expires) -->
      <div class="grid grid-cols-2 gap-3 mb-4">
        <div class="border border-om-gray-200 rounded-xl p-3.5 flex flex-col gap-2">
          <span class="text-[11px] font-semibold uppercase tracking-wider text-om-gray-400">Plan credits</span>
          <span class="text-lg font-bold text-om-gray-700 tabular-nums">{{ planRemaining.toLocaleString('en-US') }} <span class="text-xs font-medium text-om-gray-500">/ {{ planCreditsTotal.toLocaleString('en-US') }} left</span></span>
          <div class="h-1.5 rounded-full bg-om-gray-100 overflow-hidden"><div class="h-full bg-om-orange-400 rounded-full" :style="{ width: (planRemaining / planCreditsTotal * 100) + '%' }"></div></div>
          <span class="text-[11px] text-om-gray-400">Renews {{ planRenews }} · resets monthly</span>
        </div>
        <div class="border border-om-gray-200 rounded-xl p-3.5 flex flex-col gap-2">
          <span class="text-[11px] font-semibold uppercase tracking-wider text-om-gray-400">Additional credits</span>
          <span class="text-lg font-bold text-om-gray-700 tabular-nums">{{ additionalCredits.toLocaleString('en-US') }}</span>
          <span class="text-[11px] text-om-gray-400 mt-auto">Never expires · <button class="text-om-orange-500 font-semibold hover:text-om-orange-600" @click="showCreditModal = false; openBuyModal()">Buy more</button></span>
        </div>
      </div>
      <p class="text-xs text-om-gray-500 mb-4"><span class="font-semibold text-om-gray-700">Plan credits are spent first</span> — your purchased credits stay until you use them.</p>

      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          <thead>
            <tr class="text-left text-om-gray-500 border-b border-om-gray-200">
              <th class="font-normal py-2.5 pr-4">Date</th>
              <th class="font-normal py-2.5 pr-4">Type</th>
              <th class="font-normal py-2.5 pr-4">Model</th>
              <th class="font-normal py-2.5 pr-4">Credits</th>
              <th class="font-normal py-2.5 pr-4">Balance</th>
              <th class="font-normal py-2.5 pr-4">Products</th>
              <th class="font-normal py-2.5 pr-4">Tag</th>
              <th class="font-normal py-2.5 pr-4">Domain</th>
              <th class="font-normal py-2.5">User</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(t, i) in creditTransactions" :key="i" class="border-b border-om-gray-100">
              <td class="py-3 pr-4 text-om-gray-600 whitespace-nowrap">{{ t.date }}</td>
              <td class="py-3 pr-4"><Tag :variant="t.credits > 0 ? 'green' : 'orange'">{{ t.type }}</Tag></td>
              <td class="py-3 pr-4 text-om-gray-700 whitespace-nowrap">{{ t.model }}</td>
              <td class="py-3 pr-4 font-medium tabular-nums" :class="t.credits > 0 ? 'text-green-600' : 'text-om-orange-500'">{{ t.credits > 0 ? '+' + t.credits : t.credits }}</td>
              <td class="py-3 pr-4 text-om-gray-700 tabular-nums">{{ t.balance }}</td>
              <td class="py-3 pr-4 text-om-gray-700 tabular-nums">{{ t.products }}</td>
              <td class="py-3 pr-4"><span class="text-om-orange-500 whitespace-nowrap">{{ t.tag }}</span></td>
              <td class="py-3 pr-4 text-om-gray-600 whitespace-nowrap">{{ t.domain }}</td>
              <td class="py-3 text-om-gray-400 whitespace-nowrap">{{ t.user }}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="flex items-center justify-between pt-4 mt-2">
        <span class="text-sm text-om-gray-500">Showing 1-20 of 1328</span>
        <div class="flex items-center gap-2">
          <Button variant="secondary" size="sm" icon-only><template #icon><ChevronLeft :size="14" /></template></Button>
          <span class="text-sm text-om-gray-500 tabular-nums">1 / 67</span>
          <Button variant="secondary" size="sm" icon-only><template #icon><ChevronRight :size="14" /></template></Button>
        </div>
      </div>
      </Modal>
      ...
      <Modal v-model="showBuyModal" size="md" :overflow-visible="true">
        ...
        <div v-if="buyStep === 'select'" class="flex flex-col gap-4">
          <p class="text-sm text-om-gray-500">Current balance: <span class="font-semibold text-om-gray-700">{{ availableCredits.toLocaleString('en-US') }}</span> credits</p>
          <div class="flex flex-col gap-2">
            <button
            ...
            >
              <span class="flex items-center gap-2">
                <span class="text-sm font-semibold text-om-gray-700 tabular-nums">{{ pack.credits.toLocaleString('en-US') }} credits</span>
          ...
          <!-- demo: simulate saved-card scenarios -->
        ...
        <div v-else-if="buyStep === 'add-card'" class="flex flex-col gap-3">
          <p class="text-sm text-om-gray-500">Add a card to complete your purchase of <span class="font-semibold text-om-gray-700">{{ buyCredits.toLocaleString('en-US') }}</span> credits (€{{ buyPrice }}).</p>
          <div class="flex flex-col gap-1">
        ...
        <div v-else class="flex flex-col items-center justify-center gap-2 py-10 text-center">
          ...
          <p class="text-base font-semibold text-om-gray-700">+{{ lastPurchaseCredits.toLocaleString('en-US') }} credits added</p>
          <p class="text-sm text-om-gray-500">New balance: <span class="font-semibold text-om-gray-700">{{ availableCredits.toLocaleString('en-US') }}</span> credits</p>
        </div>
...
import { Search, ArrowLeft, ArrowRight, Sparkles, Image as ImageIcon, Cpu, Maximize2, Coins, Wand2, ExternalLink, Type, ArrowUpDown, SlidersHorizontal, Upload, X, Check, ChevronDown, ChevronUp, ChevronLeft, ChevronRight, Workflow, Plus, Play, Loader2, Clock, Eye, EyeOff, Receipt, CreditCard } from 'lucide-vue-next'
...
// Credit balance = renewing plan credits + never-expiring additional credits.
// Plan credits reset each billing cycle; additional (purchased) credits never expire.
const planCreditsTotal = 25000
const planCreditsUsed = ref(5000)
const planRenews = 'Aug 1'
const additionalCredits = ref(2144)
const planRemaining = computed(() => Math.max(0, planCreditsTotal - planCreditsUsed.value))
const availableCredits = computed(() => planRemaining.value + additionalCredits.value)

// Credit transactions history modal
const showCreditModal = ref(false)
const creditTransactions = ref([
  { date: '2026. 07. 07. 07:42', type: 'text', model: 'Gemini 3 Flash', credits: -20, balance: 22144, products: 10, tag: 'SDP - Product Sentence', domain: 'www.dietas-termekek-webshop.hu', user: 'Automatic' },
  { date: '2026. 07. 07. 07:42', type: 'text', model: 'Gemini 3 Flash', credits: -50, balance: 22164, products: 25, tag: 'SPo - Recommendation headline', domain: 'whiskynet.hu', user: 'Automatic' },
  { date: '2026. 07. 07. 07:42', type: 'text', model: 'Gemini 3 Flash', credits: -10, balance: 22214, products: 5, tag: 'SPPO3 - Title, description, photo', domain: 'whiskynet.hu', user: 'Automatic' },
  { date: '2026. 07. 07. 07:42', type: 'text', model: 'Gemini 3 Flash', credits: -20, balance: 22224, products: 10, tag: 'SPPO - Benefit List - livecheck', domain: 'whiskynet.hu', user: 'Automatic' },
  { date: '2026. 07. 07. 07:42', type: 'text', model: 'GPT-4o Mini', credits: -20, balance: 22244, products: 10, tag: 'SDP - Product Sentence', domain: 'whiskynet.hu', user: 'Automatic' },
  { date: '2026. 07. 07. 07:34', type: 'text', model: 'GPT-4o', credits: -45, balance: 22264, products: 5, tag: 'Benefit List Limit Test', domain: 'www.dietas-termekek-webshop.hu', user: 'Automatic' },
  { date: '2026. 07. 07. 07:29', type: 'text', model: 'Gemini 3 Flash', credits: -20, balance: 22309, products: 10, tag: 'SDP - Product Sentence', domain: 'www.dietas-termekek-webshop.hu', user: 'Automatic' },
  { date: '2026. 07. 07. 07:28', type: 'text', model: 'Gemini 3 Flash', credits: -50, balance: 22329, products: 25, tag: 'SPo - Recommendation headline', domain: 'whiskynet.hu', user: 'Automatic' },
  { date: '2026. 07. 07. 07:28', type: 'text', model: 'Gemini 3 Flash', credits: -10, balance: 22379, products: 5, tag: 'SPPO3 - Title, description, photo', domain: 'whiskynet.hu', user: 'Automatic' },
  { date: '2026. 07. 07. 07:28', type: 'text', model: 'GPT-4o Mini', credits: -20, balance: 22389, products: 10, tag: 'SDP - Product Sentence', domain: 'whiskynet.hu', user: 'Automatic' },
  { date: '2026. 07. 07. 07:28', type: 'text', model: 'Gemini 3 Flash', credits: -20, balance: 22409, products: 10, tag: 'SPPO - Benefit List - livecheck', domain: 'whiskynet.hu', user: 'Automatic' },
])

// ── Buy credits (hybrid: packs + custom amount) ──
const showBuyModal = ref(false)
const buyStep = ref('select') // 'select' | 'processing' | 'success'
const creditPacks = [
...
const openBuyModal = () => { buyStep.value = 'select'; buyCustom.value = ''; showBuyModal.value = true }
const confirmBuy = () => {
  buyStep.value = 'processing'
  setTimeout(() => {
    const credits = buyCredits.value
    additionalCredits.value += credits
    lastPurchaseCredits.value = credits
    const now = new Date()
    const p = (n) => String(n).padStart(2, '0')
    creditTransactions.value.unshift({
      date: `${now.getFullYear()}. ${p(now.getMonth() + 1)}. ${p(now.getDate())}. ${p(now.getHours())}:${p(now.getMinutes())}`,
      type: 'purchase', model: '—', credits: credits, balance: availableCredits.value,
      products: '—', tag: 'Credit purchase', domain: '—', user: 'You',
    })
