<template>
  ...
  <MainLayout :has-progress="!isDesktopRequiredStep" :is-exiting="isExitingToWizard">
    <template #content>
      <div class="w-full" @wheel="handleWheel">
        <transition :name="transitionName" mode="out-in">
          <div :key="currentStep" :class="['w-full', isExitingToWizard ? 'exit-animation' : '']">
            <!-- Back button -->
            <button
              v-if="currentStep > 1 && !isDesktopRequiredStep && !['referralSource', 'relationship', 'contactType', 'agencyDetails'].includes(currentStepKey)"
              @click="handlePrev"
    ...
    <template #progress>
      <div class="flex items-center gap-4">
        <p class="text-sm text-[#505763] whitespace-nowrap">
          {{ displayStep }} / {{ displayTotalSteps }}. lépés
        </p>
...
import StepWelcome from '../components/onboarding/StepWelcomeHu.vue'
import StepReferralSource from '../components/onboarding/StepReferralSourceHu.vue'
import StepRelationship from '../components/onboarding/StepRelationshipHu.vue'
import StepContactType from '../components/onboarding/StepContactTypeHu.vue'
import StepAgencyDetails from '../components/onboarding/StepAgencyDetailsHu.vue'
import StepDesktopRequired from '../components/onboarding/StepDesktopRequiredHu.vue'
...
const emit = defineEmits(['onboarding-complete', 'go-to-wizard', 'skip-to-dashboard', 'sign-out'])
...
const useCaseStep = { component: StepDesktopRequired, key: 'useCase', showButtons: false }
...
const isDesktopRequiredStep = computed(() => currentStepKey.value === 'useCase')
