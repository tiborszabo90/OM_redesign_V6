  ...
  <DashboardLayout active-menu-item="campaigns" @logo-click="handleLogoClick" @menu-click="$emit('menu-click', $event)" :right-panel-collapsed="!isChatOpen">
    ...
    <template #content>
      <div class="w-full max-w-[1400px] mx-auto -mt-2">
        ...
        <!-- Agentic OptiMonk: hand this campaign to the AI builder -->
        <div class="bg-white rounded-lg shadow-[0_1px_2px_1px_rgb(0_0_0/0.03)] mb-5 p-5 flex items-center gap-4">
          <div class="w-10 h-10 rounded-xl bg-om-orange-100 flex items-center justify-center shrink-0">
            <Wand2 :size="22" class="text-om-orange-500" />
          </div>
          <div class="flex-1 min-w-0">
            <h3 class="text-base font-semibold text-om-gray-700">Improve this campaign with AI</h3>
            <p class="text-sm text-om-gray-500 mt-0.5">
              Agentic OptiMonk rebuilds <span class="font-medium text-om-gray-700">{{ campaignName }}</span> from your brand and offer, then you pick the concept that fits.
            </p>
          </div>
          <Button variant="primary" size="sm" class="shrink-0" @click="showAgenticModal = true">Open in Agentic</Button>
        </div>
...
// ── Agentic OptiMonk ──
// Agentic OptiMonk (AI image-to-popup builder) lives as a separate app; the
// sidebar entry point asks to switch over to it.
