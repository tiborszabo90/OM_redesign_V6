<template>
  <DashboardLayout @logo-click="handleLogoClick" @menu-click="$emit('menu-click', $event)" :right-panel-collapsed="!isChatOpen || chatWideMode" :content-hidden="chatWideMode">
    ...
    <AgenticVariationsModal
      v-model="showVariationsModal"
      @confirm="handleVariationsConfirm"
      @select="handleVariationSelect"
    />
    <!-- Rendered last so it paints above the variations modal while that one fades out -->
    <AgenticSwitchModal v-model="showAgenticModal" @confirm="handleAgenticConfirm" />
  </DashboardLayout>
</template>

<script setup>
import { ref, reactive, computed, watch, nextTick, onMounted, onUnmounted, markRaw } from 'vue'
import { TrendingUp, TrendingDown, Calendar, UserPlus, Signpost, X, Monitor, Smartphone, Tablet, Tag, Lightbulb, ArrowUpRight, ChevronRight } from 'lucide-vue-next'
...
import AgenticVariationsModal from '../components/shared/AgenticVariationsModal.vue'
...
const props = defineProps({
  ...
  // When true (dedicated promo route), auto-open the "best campaign reimagined"
  // interstitial once on mount.
  showVariationsPromo: {
    type: Boolean,
    default: false
  }
...
// Agentic OptiMonk (AI Image-to-HTML popup builder) lives as a separate app.
...
// "Best campaign reimagined" interstitial — auto-opens once on the promo route.
// Both exits hand off to the switch confirmation first. nextTick keeps the two
// modals' scroll-lock watchers in order, so the body stays locked in between.
const showVariationsModal = ref(false)
const openAgenticSwitch = () => {
  showVariationsModal.value = false
  nextTick(() => { showAgenticModal.value = true })
}
const handleVariationsConfirm = openAgenticSwitch
const handleVariationSelect = openAgenticSwitch
...
let variationsPromoTimeout = null
onMounted(() => {
  if (props.showVariationsPromo) {
    variationsPromoTimeout = setTimeout(() => { showVariationsModal.value = true }, 1000)
  }
...
onUnmounted(() => {
  ...
  if (variationsPromoTimeout) clearTimeout(variationsPromoTimeout)
})
...
/* Agentic OptiMonk promo thumbnail */
.agentic-promo-thumb {
  position: relative;
  width: 100%;
  aspect-ratio: 16 / 9;
