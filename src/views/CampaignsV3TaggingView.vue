<template>
  <DashboardLayout active-menu-item="campaigns" @logo-click="handleLogoClick" @menu-click="$emit('menu-click', $event)" :right-panel-collapsed="!isChatOpen">
      ...
      <div class="w-full max-w-[1400px] mx-auto -mt-3">
        ...
        <div class="flex items-center gap-2 flex-wrap mb-3">
          <button
            ...
            :key="v.id"
            @click="selectView(v)"
            :class="['chip px-3 h-9 flex items-center rounded-lg text-sm cursor-pointer transition-all duration-200 ease-out', activeViewId === v.id ? 'bg-om-gray-200 text-om-gray-700 font-medium' : 'bg-om-gray-100 text-om-gray-700 hover:bg-om-gray-200']"
          >
            {{ v.label }}
          </button>
          ...
          <button
            ...
            :key="v.id"
            @click="selectView(v)"
            :class="['chip px-3 h-9 flex items-center gap-1.5 rounded-lg text-sm cursor-pointer transition-all duration-200 ease-out', activeViewId === v.id ? 'bg-om-gray-200 text-om-gray-700 font-medium' : 'bg-om-gray-100 text-om-gray-700 hover:bg-om-gray-200']"
          >
            {{ v.label }}
            <X :size="13" class="text-om-gray-400 hover:text-om-gray-700" @click.stop="deleteView(v)" />
          </button>

          <!-- Modified affordance -->
          <template v-if="isModified">
            <span class="w-px h-5 bg-om-gray-200 mx-1"></span>
            <span class="text-xs text-om-gray-400">Unsaved changes</span>
            <Button v-if="activeViewIsCustom" variant="ghost" size="sm" @click="updateActiveView">Update “{{ activeView.label }}”</Button>
            <Button variant="secondary" size="sm" @click="openSaveView">Save as new view</Button>
            <Button variant="ghost" size="sm" @click="resetToView">Reset</Button>
          </template>
        </div>
    ...
    <Modal v-model="showSaveViewModal" title="Save view" size="sm">
      <FormInput
        v-model="newViewName"
        label="View name"
      ...
      />
      <p class="text-xs text-om-gray-400 mt-2">Saves the current status and tag filters as a reusable view.</p>
      <template #footer="{ close }">
        <Button variant="secondary" @click="close">Cancel</Button>
        <Button variant="primary" :disabled="!newViewName.trim()" @click="confirmSaveView">Save view</Button>
      </template>
    </Modal>
  </DashboardLayout>
</template>
...
import { Plus, Search, Calendar, ArrowUpDown, Check, ChevronLeft, ChevronRight, Trash2, Archive, X } from 'lucide-vue-next'
...
const activeView = computed(() => savedViews.find(v => v.id === activeViewId.value))
const activeViewIsCustom = computed(() => !!activeView.value && !activeView.value.system)

const sameTags = (a, b) => a.length === b.length && a.every(t => b.includes(t))
const isModified = computed(() => activeView.value ? !sameTags(selectedTags.value, activeView.value.criteria.tags) : false)
...
const resetToView = () => { if (activeView.value) selectView(activeView.value) }
const updateActiveView = () => {
  if (activeViewIsCustom.value) activeView.value.criteria.tags = [...selectedTags.value]
}
...
const openSaveView = () => { newViewName.value = ''; showSaveViewModal.value = true }
const confirmSaveView = () => {
  const name = newViewName.value.trim()
  if (!name) return
  const id = `view-${Date.now()}`
  savedViews.push({ id, label: name, system: false, criteria: { status: activeStatusFilter.value, tags: [...selectedTags.value] } })
