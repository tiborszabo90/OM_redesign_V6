  ...
  <DashboardLayout @logo-click="handleLogoClick" @menu-click="$emit('menu-click', $event)" :right-panel-collapsed="!isChatOpen || chatWideMode" :content-hidden="chatWideMode">
    <template #content>
      <div class="w-full max-w-[1400px] mx-auto -mt-3">
        ...
        <div class="bg-white rounded-lg shadow-[0_1px_2px_1px_rgb(0_0_0/0.03)] py-10 px-12 grid grid-cols-2 items-center gap-24 mb-10">
          ...
          <div class="min-w-0 bg-om-gray-50 rounded-xl py-3 px-3 flex flex-col gap-2">
            <button
              v-for="insight in topInsights"
              :key="insight.id"
              @click="emit('navigate-to-opportunity', insight.id)"
              class="flex items-center gap-3 text-left group cursor-pointer bg-white rounded-lg px-3 py-2 hover:shadow-[0_4px_14px_rgba(0,0,0,0.08)] transition-shadow duration-200 w-full"
            >
              ...
              <span class="flex-1 text-sm text-om-gray-700 group-hover:text-om-orange-600 transition-colors">{{ insight.name }}</span>
              <ChevronRight :size="16" class="text-om-gray-400 group-hover:text-om-orange-600 shrink-0 transition-colors" />
            </button>
          </div>
        </div>
