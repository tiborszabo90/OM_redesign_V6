  ...
  <DashboardLayout @logo-click="handleLogoClick" @menu-click="$emit('menu-click', $event)" :right-panel-collapsed="!isChatOpen || chatWideMode" :content-hidden="chatWideMode">
    <template #content>
      <div class="w-full max-w-[1400px] mx-auto -mt-3">
        ...
        <!-- Alerts -->
        <div v-if="alerts.length" class="flex flex-col gap-3 mb-5">
          <div
          ...
          >
            ...
            <div class="absolute top-2 right-2">
              <Button variant="ghost" size="sm" icon-only @click="dismissAlert(alert.id)">
...
const alerts = ref([
  {
    id: 'wp-plugin-missing',
    title: 'Install the OptiMonk plugin for full functionality',
    body: [
      { text: 'You are not currently using the ' },
      { text: 'OptiMonk plugin', href: '#' },
      { text: ' on your WordPress site. Install the plugin to unlock the latest features. After installing, make sure you have granted the required access on the ' },
      { text: 'Code insertion page', href: '#' },
      { text: ' so that new and upcoming features work properly.' },
    ],
  },
])
