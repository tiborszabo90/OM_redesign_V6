        ...
        <div class="flex items-start justify-between mb-4">
          <div>
            ...
            <div class="flex items-center gap-1.5 flex-wrap mt-2">
              <Tag v-for="t in tags" :key="t" :color="tagColor(t)" size="sm">
                {{ t }}
                <button class="ml-0.5 inline-flex hover:opacity-60 cursor-pointer" @click="removeTag(t)">
                  <X :size="10" />
                </button>
              </Tag>
              <div class="relative">
                ...
                <div v-if="tagMenuOpen" class="fixed inset-0 z-10" @click="tagMenuOpen = false"></div>
                <div v-if="tagMenuOpen" class="absolute left-0 top-full mt-1 z-20 bg-white border border-[#D5D8DD] rounded-lg shadow-lg w-52 p-2">
                  <input
                    v-model="newTagInput"
                  ...
                  <div class="max-h-40 overflow-y-auto">
                    ...
                    >
                      ...
                      <Check v-if="tags.includes(t)" :size="12" class="text-om-gray-500 shrink-0" />
                    </button>
                    <button
...
const toggleTagMenu = () => { tagMenuOpen.value = !tagMenuOpen.value; newTagInput.value = '' }
const toggleTag = (tag) => {
  const i = tags.value.indexOf(tag)
  if (i === -1) tags.value.push(tag); else tags.value.splice(i, 1)
}
const removeTag = (tag) => {
  const i = tags.value.indexOf(tag)
