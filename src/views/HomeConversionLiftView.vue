<template>
  <DashboardLayout :nav-items="navItems" logo-src="/conversion_lift_logo_white_by_optimonk%201.svg" @logo-click="handleLogoClick" @menu-click="$emit('menu-click', $event)" :right-panel-collapsed="!isChatOpen || chatWideMode" :content-hidden="chatWideMode">
    <template #content>
...
import { TrendingUp, TrendingDown, Target, Calendar, UserPlus, Signpost, X, Monitor, Smartphone, Tablet, MessageCircle, MousePointerClick, Filter, FlaskConical, Clock, RotateCw, Facebook, Search, Route, Tag, Sparkles, LayoutGrid, Layers, Info, ClipboardList, ShieldAlert, Eye, Wand2, ArrowDown, BarChart3, ShieldCheck, Globe, Truck, Lightbulb, ArrowUpRight, ChevronRight, Home, Users, BookOpen, GitBranch, LayoutTemplate, PanelsTopLeft } from 'lucide-vue-next'
...
import { clNavItems, clNavItemsWithFlyout } from '../data/clNavItems.js'
...
const props = defineProps({
  ...
  // When true, the "Conversion Lift" menu item exposes its products as a flyout
  // submenu (the home dashboard stays as the background).
  flyoutProducts: {
    type: Boolean,
    default: false
  }
...
// Sidebar menu: same as the default dashboard, but "Insights" is replaced by "Conversion Lift".
// In flyout mode the Conversion Lift item carries its products as submenu children.
const navItems = computed(() => (props.flyoutProducts ? clNavItemsWithFlyout : clNavItems))
