          ...
          <div class="flex flex-col gap-3">
            ...
            >
              ...
              <div class="flex gap-4 items-stretch">
                ...
                <!-- Instructions column -->
                <div class="flex-1 min-w-0 flex flex-col pt-8 pb-10">
                  <textarea
                  ...
                  />
                  <div class="mt-3">
                    <span class="block text-xs text-om-gray-500 mb-1">History</span>
                    <Dropdown
                      :model-value="getCurrentImage(product)"
                      :options="getHistory(product)"
                      size="sm"
                      drop-up
                      @update:model-value="(v) => setCurrentImage(product, v)"
                    >
                      <template #selected="{ option }">
                        <span class="flex items-center gap-2 min-w-0">
                          <img :src="option?.src" class="w-5 h-5 rounded object-cover shrink-0" />
                          <span class="truncate">{{ option?.label }}</span>
                        </span>
                      </template>
                      <template #option="{ option }">
                        <span class="flex items-center gap-2 min-w-0">
                          <img :src="option.src" class="w-6 h-6 rounded object-cover shrink-0" />
                          <span class="truncate">{{ option.label }}</span>
                        </span>
                      </template>
                    </Dropdown>
...
import { Search, ArrowLeft, ArrowRight, Sparkles, Image as ImageIcon, Cpu, Maximize2, Coins, Wand2, ExternalLink, Type, ArrowUpDown, SlidersHorizontal, Upload, X, Check, ChevronDown, ChevronUp, ChevronLeft, ChevronRight, Eye, EyeOff } from 'lucide-vue-next'
...
const genReviewFilterCount = computed(() => (genCreatedFrom.value || genCreatedTo.value) ? 1 : 0)
