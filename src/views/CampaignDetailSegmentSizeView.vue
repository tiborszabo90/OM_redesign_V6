      ...
      <div class="w-full max-w-[1400px] mx-auto -mt-2">
        ...
        <div v-if="activeTab === 'Settings'" class="space-y-4 pb-40">
          ...
          >
            ...
            <SegmentSizeBanner
              class="mb-5"
              :percent="segmentEstimate.percent"
              :visitors="segmentEstimate.visitors"
              :total-visitors="segmentEstimate.totalVisitors"
              :is-upper-bound="segmentEstimate.isUpperBound"
              :unestimable-rules="segmentEstimate.unestimableRules"
              :estimable-count="segmentEstimate.estimableCount"
              :total-count="segmentEstimate.totalCount"
            />
            <div class="trigger-timeline">
              <div class="trigger-timeline-item">
                <div class="trigger-card">
                  <div class="flex items-center gap-3">
                    ...
                    <RuleReachChip v-bind="ruleReach('Spent on pages')" />
                  </div>
                </div>
              </div>
              <div class="trigger-timeline-item">
                <div class="trigger-card">
                  <div class="flex items-center gap-3">
                    ...
                    <RuleReachChip v-bind="ruleReach('Current page / URL')" />
                  </div>
                </div>
              </div>
              <div v-for="(rule, i) in addedTargetingRules" :key="`added-rule-${i}`" class="trigger-timeline-item">
                <div class="trigger-card">
                  <div class="flex items-center gap-3">
                    ...
                    <RuleReachChip v-bind="ruleReach(rule.title)" />
...
import SegmentSizeBanner from '../components/shared/SegmentSizeBanner.vue'
import RuleReachChip from '../components/shared/RuleReachChip.vue'
...
// ── Segment size estimate (mock) ──
// Last-7-day traffic base used for the estimate.
const WEEKLY_TRAFFIC = 24800
// Per-rule reach: `estimable` = we have logged 7-day data to estimate it;
// `reachFactor` = share of traffic (0–1) that passes the rule. Behavioral /
// un-logged rules are not estimable and only trigger the warning.
const RULE_REACH = {
  'Current page / URL': { estimable: true, reachFactor: 0.18 },
  'Visited URL': { estimable: true, reachFactor: 0.30 },
  'Number of visited pages': { estimable: true, reachFactor: 0.50 },
  'Country': { estimable: true, reachFactor: 0.55 },
  'Source': { estimable: true, reachFactor: 0.42 },
  'Returning / New': { estimable: true, reachFactor: 0.60 },
  'Subscribers / Non-subscribers': { estimable: true, reachFactor: 0.70 },
  'Spent on pages': { estimable: false },
  'Spent on site': { estimable: false },
  'Engaged with OptiMonk campaigns': { estimable: false },
  'Cart rules': { estimable: false },
  'Cookie segmentation': { estimable: false },
  'AdBlock detection': { estimable: false },
  'Block IP addresses': { estimable: false },
  'Exclude page visitors': { estimable: false },
  'Other visitor attributes': { estimable: false },
}
// Active rules = the two seeded rules + anything added via the modal.
const activeTargetingRules = computed(() => [
  { title: 'Spent on pages' },
  { title: 'Current page / URL' },
  ...addedTargetingRules.value,
])

const segmentEstimate = computed(() => {
  const rules = activeTargetingRules.value
  const estimable = rules.filter((r) => RULE_REACH[r.title]?.estimable)
  // Unknown titles default to un-estimable so they surface in the warning.
  const unestimable = rules.filter((r) => !RULE_REACH[r.title]?.estimable)
  const factor = estimable.reduce((acc, r) => acc * RULE_REACH[r.title].reachFactor, 1)
  const visitors = Math.round(WEEKLY_TRAFFIC * factor)
  return {
    percent: Math.round(factor * 100),
    visitors,
    totalVisitors: WEEKLY_TRAFFIC,
    isUpperBound: unestimable.length > 0,
    unestimableRules: [...new Set(unestimable.map((r) => r.title))],
    estimableCount: estimable.length,
    totalCount: rules.length,
  }
})

// The estimable rule that narrows traffic the most (lowest reachFactor) — the
// best starting point when you want to loosen targeting.
const strongestFilterTitle = computed(() => {
  let best = null
  for (const r of activeTargetingRules.value) {
    const meta = RULE_REACH[r.title]
    if (meta?.estimable && (!best || meta.reachFactor < best.factor)) {
      best = { title: r.title, factor: meta.reachFactor }
    }
  }
  return best?.title ?? null
})

// Per-rule reach info for the chip shown on each rule card.
const ruleReach = (title) => {
  const meta = RULE_REACH[title]
  if (!meta?.estimable) return { measured: false }
  return {
    measured: true,
    keptPercent: Math.round(meta.reachFactor * 100),
    strongest: title === strongestFilterTitle.value,
  }
}
