<template>
  ...
  <DashboardLayout active-menu-item="campaigns" @logo-click="handleLogoClick" @menu-click="$emit('menu-click', $event)" :right-panel-collapsed="!isChatOpen">
    <!-- Agentic OptiMonk entry point, above the notification bell -->
    <template #sidebar-bottom>
      <button
        title="Try Agentic OptiMonk"
        class="group w-10 h-10 rounded-full flex items-center justify-center bg-om-orange-100 text-om-orange-500 hover:bg-om-orange-200 transition-colors cursor-pointer"
        @click="showAgenticModal = true"
      >
        <Wand2 :size="22" class="transition-transform duration-200 group-hover:-rotate-12" />
      </button>
    </template>
    <template #content>
  ...
  <!-- Enlarged concept preview, then the switch confirmation -->
  <AgenticConceptPreviewModal
    v-model="showConceptPreview"
    :concept="previewConcept"
    :index="previewIndex"
    :total="agenticConcepts.length"
    @confirm="handleConceptConfirm"
  />
  <!-- Rendered last so it paints above the preview modal while that one fades out -->
  <AgenticSwitchModal v-model="showAgenticModal" @confirm="handleAgenticConfirm" />
  </div>
</template>

<script setup>
import { ref, computed, watch, onMounted, onUnmounted, nextTick } from 'vue'
import { ChevronDown, ChevronRight, TrendingUp, TrendingDown, AlertTriangle, Calendar, Target, MoreVertical, GraduationCap, Clock, RefreshCw, Users, Send, Monitor, Smartphone, X, ChevronsUp, ChevronsDown, Minus, Check, Globe, Plus, FlaskConical, Mail, Search, ArrowUpDown, Columns3, LogOut, Timer, Zap, Ban, Hourglass, MessageCircle, MousePointerClick, Filter, RotateCw, Facebook, Route, Tag as TagIcon, Sparkles, LayoutGrid, Layers, Info, ClipboardList, ShieldAlert, Eye, Wand2, ArrowDown, ZoomIn, BarChart3, ShieldCheck, Truck } from 'lucide-vue-next'
...
import AgenticSwitchModal from '../components/shared/AgenticSwitchModal.vue'
import AgenticConceptPreviewModal from '../components/shared/AgenticConceptPreviewModal.vue'
...
// ── Agentic OptiMonk: AI concepts for this campaign ──
// Agentic OptiMonk (AI image-to-popup builder) lives as a separate app.
const showAgenticSection = ref(true)
const showAgenticModal = ref(false)
const handleAgenticConfirm = () => {
...
const showConceptPreview = ref(false)
const previewIndex = ref(0)
const previewConcept = computed(() => agenticConcepts[previewIndex.value] || null)
const openConceptPreview = (i) => {
  previewIndex.value = i
  showConceptPreview.value = true
}
const handleConceptConfirm = () => {
  showConceptPreview.value = false
  nextTick(() => { showAgenticModal.value = true })
}
