...
import { state, products, styleOptions, styleById, placementOptions, variationBatches } from '../store'
import StyledImage from '../components/StyledImage.vue'
import { Layers, Lock, Plus, ArrowRight, ArrowLeft, RefreshCw, Shuffle, Check, Pause, ChevronRight } from 'lucide-vue-next'
  ...
  <div v-else-if="currentBatch" class="max-w-[960px] mx-auto px-6 py-5">
    ...
    <div class="flex flex-col gap-3">
      <div v-for="p in batchProducts" :key="p.id" class="pb-card px-4 py-3 flex items-center gap-4">
        ...
        <button class="pb-btn-ghost shrink-0" @click="switchLook(p.id)" title="Switch look">
          <Shuffle :size="15" />
        </button>
        <button class="pb-btn-ghost shrink-0" @click="regenerate(p.id)" title="Regenerate">
          <RefreshCw :size="15" />
        </button>
        <button
