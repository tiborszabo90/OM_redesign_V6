  ...
  <div class="relative w-full h-full overflow-hidden">
    ...
    <template v-if="overlay === 'callouts'">
      <template v-if="compact">
        <span class="absolute left-1.5 top-[22%] w-2 h-2 rounded-full bg-white shadow ring-2 ring-[#b2592e]"></span>
        <span class="absolute right-1.5 bottom-[26%] w-2 h-2 rounded-full bg-white shadow ring-2 ring-[#b2592e]"></span>
      </template>
      <template v-else>
        <span class="absolute left-3 top-[22%] inline-flex items-center gap-1.5 bg-white/95 rounded-full px-2.5 py-1 text-[12px] font-semibold text-[#1a1a1a] shadow">
          <span class="w-2 h-2 rounded-full bg-[#b2592e]"></span>{{ callouts[0] }}
        </span>
        <span class="absolute right-3 bottom-[26%] inline-flex items-center gap-1.5 bg-white/95 rounded-full px-2.5 py-1 text-[12px] font-semibold text-[#1a1a1a] shadow">
          <span class="w-2 h-2 rounded-full bg-[#b2592e]"></span>{{ callouts[1] }}
        </span>
      </template>
    </template>
  </div>
