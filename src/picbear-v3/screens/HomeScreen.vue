...
<template>
  <div class="max-w-[960px] mx-auto px-6 py-5">
    ...
    <!-- The four previews, always here: they are what the first step is about -->
    <div class="pb-card p-4 mb-4">
      <div class="flex items-start justify-between gap-4 mb-3">
        <div>
          <p class="font-semibold text-[#1a1a1a]">Your bestseller in 4 looks</p>
          <p class="text-[12px] text-[#616161] mt-0.5">{{ bestseller.name }}, already rendered in every style.</p>
        </div>
        <button class="pb-btn-primary shrink-0" @click="startSetup">
      ...
      <div class="grid grid-cols-4 gap-3">
        <div
          ...
          class="aspect-square rounded-xl overflow-hidden border border-[#ececec] cursor-pointer"
          @click="startSetup"
        >
          <img :src="opt.preview" :alt="opt.name" class="w-full h-full object-cover transition-transform duration-200 hover:scale-[1.02]" />
        </div>
      </div>
    </div>

    <!-- Inspiration feed, always here -->
    <div class="pb-card p-4 mb-4">
      <InspirationFeed />
    </div>
